										...
										<xsl:choose>
											<xsl:when test="following::tei:anchor[@id = $addSpanId3] or following::tei:anchor[@id = $addSpanId4]">
												<span class="tooltiptrigger ttFoot">
													<img class="keyword_symbol" src="images/keyword_symbol.gif"/>
												</span>
												<span class="tooltip ttFoot">
													<span class="ttFixed">
														<xsl:apply-templates/>
...
<xsl:apply-templates/>
	...
	<xsl:template match="tei:reg">
		<xsl:choose>
			...
			<xsl:when test="@type = 'empty'">
				<!-- Use this to insert the squared times symbol as a character instead of as an image file:
				<span class="reg_hide tooltiptrigger ttNormalisations">&#x22A0;</span>
				-->
